...
EntityMyMovement = ig.Entity.extend({
	...
	gravitable: false, // okresla czy: jednostka podlega grawitacji (zonk, infotron)
	...
	sharesTileWith: function (other) {
		var ts = this.tileSize;
		var g1 = this.getGridPos();
		var g2 = other.getGridPos ? other.getGridPos() : {
			x: Math.floor(other.pos.x / ts) * ts,
			y: Math.floor(other.pos.y / ts) * ts
		};
		return g1.x === g2.x && g1.y === g2.y;
	},

	getMoveDestination: function () {
		if (this.vel.x === 0 && this.vel.y === 0) {
	...
	// collect only on destination tile while moving, or current tile when idle
	isCollectingAt: function (entity) {
		var ts = this.tileSize;
		var tileX = Math.floor(entity.pos.x / ts) * ts;
		var tileY = Math.floor(entity.pos.y / ts) * ts;
		var dest = this.getMoveDestination();
		return dest.x === tileX && dest.y === tileY;
	},
	...
	getSupportEntityBelow: function () {
		var cell = this.getNeighborCellPos(8);
		var ents = this.getEntitiesInCell(cell.x, cell.y);

		for (var i = 0; i < ents.length; i++) {
			var ent = ents[i];
			if (this.isSupportingEntity(ent)) {
				return ent;
			}
		}
	...
	isCellBlockedForGravity: function (check) {
		...
		for (var i = 0; i < ents.length; i++) {
			...
			if (this.isSupportingEntity(ent) || ent.gravitable ||
	...
	canFallToCell: function (check) {
		if (this.isCellBlockedForGravity(check)) {
			return false;
		}
	...
	isCellSupported: function (check) {
		if (check === 8) {
			return this.getSupportEntityBelow() !== null;
		}
	...
	isRoundedSupportBelow: function () {
		var support = this.getSupportEntityBelow();
		return !!(support && support.rounded);
	},

	// side + below-diagonal must be void (Supaplex manual)
	isRollCellClear: function (check) {
		var cell = this.getNeighborCellPos(check);
		var ts = this.tileSize;
		var map = ig.game.collisionMap;

		if (map.getTile(cell.x + ts * 0.5, cell.y + ts * 0.5)) {
			return false;
		}
		...
		for (var i = 0; i < ents.length; i++) {
			...
			if (ent.consumable || ent.gravitable ||
				ent.collides === ig.Entity.COLLIDES.FIXED ||
				ent.collides === ig.Entity.COLLIDES.ACTIVE) {
				return false;
			}
	...
	canRollLeft: function () {
		return this.isRollCellClear(4) &&
			this.isRollCellClear(7) &&
			!this.isCellBlockedForGravity(4) &&
			this.canStepToCell(4);
	},

	canRollRight: function () {
		return this.isRollCellClear(6) &&
			this.isRollCellClear(9) &&
			!this.isCellBlockedForGravity(6) &&
			this.canStepToCell(6);
	},

	trySlipOff: function () {
		if (!this.gravitable || this.vel.x !== 0 || this.vel.y !== 0) {
			return false;
		}
		if (!this.isRoundedSupportBelow()) {
			return false;
		}
		if (this.canRollLeft()) {
			this.gotoGrid('left');
	...
	updateGravity: function () {
		...
		if (!this.isCellSupported(8)) {
			if (this.canFallToCell(8)) {
				this.gotoGrid('down');
				return;
			}
		}
	...
	checkFallCollisions: function () {
		...
		for (var i = 0; i < ig.game.entities.length; i++) {
			...
			var otherBottom = other.pos.y + other.size.y;

			// Only crush when vertically adjacent or overlapping, not same column far away
			if (rockTop > otherBottom || rockBottom < otherTop) {
				continue;
			}
	...
	updateGridMovement: function () {
		...
		if (this.gravitable && this.direction !== "up") {
			this.finishGravitableMove();
		}
	},

	finishGravitableMove: function () {
		if (this.vel.x !== 0 || this.vel.y !== 0) {
			return;
		}
		if (!this.isCellSupported(8) && this.canFallToCell(8)) {
			this.gotoGrid('down');
			return;
		}
		this.trySlipOff();
