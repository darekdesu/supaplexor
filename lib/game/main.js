...
SupaplexGame = ig.Game.extend({
	...
	update: function() {
		...
		if (this.mode === SupaplexGame.MODE.DEATH_PENDING && this.deathScreenPos) {
			this.screen.x = this.deathScreenPos.x - ig.system.width / 2;
			this.screen.y = this.deathScreenPos.y - ig.system.height / 2;
		} else {
	...
	updateGravity: function () {
		var gravitables = this.getEntitiesByType(EntityZonk)
			.concat(this.getEntitiesByType(EntityInfotron));

		gravitables.sort(function (a, b) {
			return b.pos.y - a.pos.y;
		});

		for (var i = 0; i < gravitables.length; i++) {
			gravitables[i].updateGravity();
		}
	},
    ...
    loadLevel: function (level, keepDeathCount) {
        this.currentLevel = level;
        var deaths = keepDeathCount ? this.deathCount : 0;
        ...
        this.infotronTotal = this.getEntitiesByType(EntityInfotron).length;

        this.levelTime = new ig.Timer();
        this.deathScreenTimer = null;
        this.deathScreenPos = null;
    ...
    restartCurrentLevelAfterDeath: function () {
    ...
    run: function () {
        ...
        if (this.mode == SupaplexGame.MODE.GAME || this.mode == SupaplexGame.MODE.DEATH_PENDING) {
            this.update();
            this.draw();
            if (this.mode == SupaplexGame.MODE.DEATH_PENDING && this.deathScreenTimer.delta() >= 0) {
    ...
    showStatsKill: function () {
        if (ig.input.pressed('enter')) {
            this.restartCurrentLevelAfterDeath();
            return;
        }
    ...
    scheduleDeathScreen: function (pos) {
        if (this.mode !== SupaplexGame.MODE.GAME) {
            return;
        }

        this.deathScreenPos = pos || null;

        this.levelTimeText = this.levelTime.delta().round(2).toString();
        this.levelTime.pause();
        this.deathCount++;
        this.deathScreenTimer = new ig.Timer(0.8 + 0.1);
        this.mode = SupaplexGame.MODE.DEATH_PENDING;
    }
});
